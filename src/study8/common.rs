...
use super::{transforms, vertex_data};
...
#[allow(dead_code)]
pub fn vertex(p:[i8;3], n:[i8; 3], dt: f32) -> Vertex {
    ...
    let noise = [
        perlin.get([1.0, dt]) as f32 * 2.0, 
        perlin.get([1.0, dt]) as f32 * 2.0, 
        perlin.get([1.0, dt]) as f32 * 2.0, 
        perlin.get([1.0, dt]) as f32 * 0.3];
    Vertex {
...
fn create_vertices(dt: f32) -> Vec<Vertex> {
    ...
    for i in 0..pos.len() {
        ...
        data.push(vertex(pos[i], col[i], (dt * count as f32).sin()));
...
impl State {
    async fn new(window: &Window, light_data: Light) -> Self {        
        let init =  transforms::InitWgpu::init_wgpu(window).await;
        let vertex_data = create_vertices(0.0);
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        self.vertex_buffer = self.init.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Vertex Buffer"),
            contents: cast_slice(&create_vertices(dt*3.0)),
            usage: wgpu::BufferUsages::VERTEX,
        });
...
pub fn run(light_data: Light, title: &str) {
    ...
    let mut state = pollster::block_on(State::new(&window, light_data));    
