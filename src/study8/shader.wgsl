...
@vertex
fn vs_main(@location(0) pos:vec4<f32>, @location(1) normal:vec4<f32>, @location(2) random: vec4<f32>)
-> Output{
    var output:Output;
    let m_position:vec4<f32> = uniforms.model_mat * pos + random;
...
struct LightUniforms {
    ...
    sppecular_shininess: f32
}
@binding(2) @group(0) var<uniform> light_uniforms: LightUniforms;

@fragment
fn fs_main(@location(0) v_position: vec4<f32>, @location(1) v_normal:vec4<f32>)
-> @location(0) vec4<f32> {
    ...
    let diffuse:f32 = light_uniforms.diffuse_intensity * max(dot(N, H), 0.0);
    let specular: f32 = light_uniforms.specular_intensity * pow(max(dot(N, H), 0.0), light_uniforms.sppecular_shininess);
